...
class EncrytSend() :
    ...
    def mic_callback(self,in_data, frame_count, time_info, status):
        ...
        if status == pyaudio.paInputOverflow:
            ...
            self.data=self._buff.get_nowait()
            #print("데이터 넣음 : "+print(len(str(self._buff))))
            #encrypted = enc.encrypt(data)
            # self.sock.sendto(in_data, ("192.168.1.112", 10001))
            # print("Data sent:", len(in_data))
            # print("Data sent:", in_data)
    ...
    def SendStart(self,buff,cipher) :
            ...
            try:
                ...
                # print(data)
                # self.sock.sendto(data, ("192.168.1.34", 10001))
                #if buff.qsize() < 100:
                #while not buff.empty():
                #data = self._buff.get_nowait()
                    #data = b"test"
                        # Opus 인코딩 (정상적인 데이터일 경우만)
                #if self.data:
                ...
                self.sock.sendto(encrypted, ("192.168.1.112", 10001))
                print("Data sent:", len(encrypted))
                    # while True :
                    #     data = buff.get_nowait()
                        # 큐에 데이터 다 처리
                ...
                print(e)
    def SendPreparation(self,id,myid):
        try:
            #with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s :
                ...
                # s.close()
            sio = socketio.Client()
            sio.connect("http://192.168.1.112:34567",transports=["websocket"])
...
class EncrytRec:
    ...
    CHANNELS : 음성(마이크) 채널
    """
    def __init__(self, UDP_IP="0.0.0.0", UDP_PORT=8080, CHANNELS=1):
        ...
        self.sock.settimeout(1)
        try:
            self.sock.bind(("192.168.1.10", 10001))
            print("[RecStart] 바인딩 성공, 수신 시작")
        except Exception as e:
            print("[RecStart] 바인딩 실패:", e)
            return
    ...
    def RecStart(self):
        # with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock :
            ...
            # try:
            #     print("test")
            #     data, _ = self.sock.recvfrom(self.CHUNK * self.CHANNELS * 2)
            #     print("test")
            ...
            #return True
        #self.sock.setblocking(True)
        #self.sock.settimeout(0.5)  # 0.5초마다 한 번씩 timeout 예외 발생
        ...
        # while True:
        try:
            data, _ = self.sock.recvfrom(self.CHUNK * self.CHANNELS)
            print("[RecStart] 데이터 수신됨")
            print("data : "+str(data))
            self.stream.write(data)
            ...
            print(e)
        # await asyncio.sleep(1)  # CPU 100% 방지    
